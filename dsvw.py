#!/usr/bin/env python3
import http.server, cgi, io, http.client, json, os, pickle, random, re, socket, socketserver, sqlite3, string, sys, subprocess, time, traceback, urllib.request, urllib.parse, urllib.error, xml.etree.ElementTree
try:
    import lxml.etree
except ImportError:
...
LISTEN_ADDRESS, LISTEN_PORT = "0.0.0.0", 1024
        ...
        try:
            if path == '/':
                if "id" in params:
                    ...
                    content = (open(os.path.abspath(params["path"]), "rb") if not "://" in params["path"] else urllib.request.urlopen(params["path"])).read()
                elif "domain" in params:
                    content = subprocess.check_output("nslookup " + params["domain"], shell=True, stderr=subprocess.STDOUT, stdin=subprocess.PIPE)
                elif "xml" in params:
                    content = lxml.etree.tostring(lxml.etree.parse(io.StringIO(params["xml"]), lxml.etree.XMLParser(no_network=False)), pretty_print=True)
                elif "name" in params:
                    found = lxml.etree.parse(io.StringIO(USERS_XML)).xpath(".//user[name/text()='%s']" % params["name"])
